...
filter.cpg.data = function(cg, minlength, maxlength, maxOE, minOE){
...
plot.stacked.lines = function(spp.list, color.list, xlim, ylim){
	...
	axis(1, cex.axis = 1)
	axis(2, las = 1, cex.axis = 1, at = signif(seq(from = min(ylim), to = max(ylim), by = (max(ylim) - min(ylim))/3), digits = 2))
	rho.values = c()
	p.values = c()
	...
	title(main = paste(round(mean(rho.values), 2), round(mean(p.values), 5)))
}
...
plot.lm = function(x, y, df, xlab, ylab, main, limits, xlim, ylim){
	...
	if (limits == T){
		...
		axis(1, cex.axis = cex.axis)
		axis(2, at = signif(seq(from = ylim[1], to = ylim[2], by = (ylim[2] - ylim[1])/2), digits = 2))
		} else {
		plot(df[,y] ~ df[,x], xlab = xlab, ylab = ylab, main = paste("rho = ", rho), col = alpha('grey', alpha), xlim = rev(range(df[,x])))
		}
 	lm1 = lm(df[,y] ~ df[,x])
 	print(summary(lm1))
 	abline(lm1, col = 'red')
