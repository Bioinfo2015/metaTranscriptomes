...
for (i in ps){
  ...
  comb$log2FoldChange = comb$log2FoldChange*-1#this is because reversing the X axis screws up plotCI()
  means = append(means, mean(comb$log2FoldChange))
  st.errs = append(st.errs, std.error(comb$log2FoldChange))
...
head(cpg.dat)
head(pdat)
plot(log2FoldChange~cpgOE, data = cpg.dat)
abline(v = .65)
abline(h = 0)
abline(v = .8)
abline(h = 1)
cpg.cut = .65
mbd.cut = 0
new.sub = cpg.dat[cpg.dat$cpgOE > cpg.cut & cpg.dat$log2FoldChange > mbd.cut,]
mbd.methylated = cpg.dat[cpg.dat$log2FoldChange > 0,]
new.count = nrow(new.sub)
not.new.count = nrow(mbd.methylated) - new.count
ps

hi.cpg = cpg.dat[cpg.dat$cpgOE > cpg.cut,]
new = hi.cpg[hi.cpg$log2FoldChange > mbd.cut,]
never = hi.cpg[hi.cpg$log2FoldChange < mbd.cut,]
plot(log2FoldChange~cpgOE, data = cpg.dat)
points(log2FoldChange~cpgOE, data = new.meth, col = 'green')
points(log2FoldChange~cpgOE, data = never.meth, col = 'blue')
...
cpg.hi.cut = .65
cpg.low.cut = .65
plot(log2FoldChange~cpgOE, data = cpg.dat, col = "grey75")
...
hi.mbd = cpg.dat[cpg.dat$log2FoldChange >= mbd.hi.cut,]
low.mbd = cpg.dat[cpg.dat$log2FoldChange < mbd.low.cut,]
new = hi.cpg[hi.cpg$log2FoldChange > mbd.hi.cut,]
never = hi.cpg[hi.cpg$log2FoldChange < mbd.low.cut,]
old = cpg.dat[cpg.dat$log2FoldChange >= mbd.hi.cut & cpg.dat$cpgOE <= cpg.low.cut,]
miss = cpg.dat[cpg.dat$log2FoldChange < mbd.low.cut & cpg.dat$cpgOE < cpg.low.cut,]

#plot the quadrants to prove it worked
plot(log2FoldChange~cpgOE, data = cpg.dat, col = "grey75")
points(log2FoldChange~cpgOE, data = hi.mbd, col = 'red') #HI MBD
points(log2FoldChange~cpgOE, data = low.mbd, col = 'orange') #LOW MBD
points(log2FoldChange~cpgOE, data = hi.cpg, col = 'grey75') #HI CPG
points(log2FoldChange~cpgOE, data = low.cpg, col = 'grey75') #LOW CPG
points(log2FoldChange~cpgOE, data = new, col = 'green') #NEW = GREEN
points(log2FoldChange~cpgOE, data = never, col = 'blue') #NEVER = BLUE
points(log2FoldChange~cpgOE, data = old, col = 'black') #OLD = BLACK
points(log2FoldChange~cpgOE, data = miss, col = 'purple') #MISS = PURPLE
...
quartz()
plot(grandmeans~log2FoldChange, data = sdat, col = 'grey', xlim = rev(range(sdat$log2FoldChange)))
window_data = window_plot(1/10, 'log2FoldChange', 'grandmeans', sdat, Cutoff) ## data for Carly's RNA seq
...
plot.meth.bars(sdat, 'log2FoldChange', 'grandmeans', 0, c('black', 'darkgreen'))



#plot cloud with sanity check overlaid
QUANTILE = F
plot(sanity~log2FoldChange, data = sdat, col = 'grey', xlim = rev(range(sdat$log2FoldChange)), xlab = "MBD-score", ylab = "Transcript Abundance")
window_data = window_plot(1/10, 'log2FoldChange', 'sanity', sdat, Cutoff) ## data for Carly's RNA seq
window_data = na.omit(window_data)##remove the windows that had no genes in them
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = T, scol = 'black', col = 'black', lwd = 2, pch = ".", cex = .01, xlim = rev(range(window_data$x)))
...
#plot zooms
QUANTILE = T
window_data = window_plot(1/20, 'log2FoldChange', 'grandmeans', sdat, Cutoff) ## data for Carly's RNA seq
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = F, scol = 'black', col = 'black', lwd = 3, pch = 26, cex = .01, xlim = rev(range(window_data$x)), xlab = "log2FoldChange", ylab = "Transcript Abundance")
window_data = window_plot(1/20, 'log2FoldChange', 'sanity', sdat, Cutoff) ## data for Carly's RNA seq
...
# plot.meth.bars(sdat, 'log2FoldChange', 'sanity', 0, c('black', 'darkgreen'))
# plot.meth.bars(sdat, 'log2FoldChange', 'sanity', 0, c('orange', 'orange'))
...
weak = tdat[tdat$log2FoldChange < 0,]$grandmeans
strong = tdat[tdat$log2FoldChange > 0,]$grandmeans
mn.w = mean(weak)
st.w = std.error(weak)
mn.s = mean(strong)
...
x = sample(sdat$log2FoldChange, length(sdat$log2FoldChange))
sdat$sanity = x
sdat = sdat[with(sdat, order(log2FoldChange)), ]
loess_fit <- loess(log2FoldChange~grandmeans, sdat, span = .2, se = T)
lines(sdat$log2FoldChange, predict(loess_fit),col="red",lwd=3)
plot(grandmeans~log2FoldChange, data = sdat, col = 'grey75')
...
# ?clusplot()
# ?pam
# x = data.frame(sdat$log2FoldChange, sdat$grandmeans)
...
# 
# plot(log2FoldChange~cpgOE, data = dat)
...
quartz()
par(mfrow = c(2,1))
hist(mdat$log2FoldChange, breaks = 50, prob = F, xlim = c(4,-4), ylab = 'Genes', main = '', xlab = 'Strength of Methylation')

plot.counts(ott, 'log2FoldChange', 'adj.p', 0.01, 1/25)
axis(1, at = c(4, 2, 0, -2))
axis(2, at = c(10, 20, 30))
box()
window_data = window_plot(1/15, 'log2FoldChange', 'absDiff', ott, 10) ## data for Carly's RNA seq
plot(abs(log2.difference)~log2FoldChange, data = ott, xlim = rev(c(-3.5, 6.2)), col = 'grey', ylim = c(0, 2))
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = T, scol = 'black', lwd = 1, pch = 19, cex = 0.01)
lm1 = lm(abs(log2.difference)~log2FoldChange, data = ott)
abline(lm1, col = 'red')
plot(abs(log2.difference)~log2FoldChange, data = ott, xlim = rev(c(-2,2)), ylim = c(.23,.37), col = 'grey')
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = T, scol = 'black', lwd = 1, pch = 19, cex = 0.01)
#BULID THE POPULATION-WISE PLOTS
plot.counts(oot, 'log2FoldChange', 'adj.p', 0.01, 1/25)
window_data = window_plot(1/15, 'log2FoldChange', 'absDiff', oot, 10) ## data for Carly's RNA seq
plot(absDiff~log2FoldChange, data = oot, xlim = rev(c(-3.5, 6.2)), col = 'grey', ylim = c(0, 2))
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = T, scol = 'black', lwd = 1, pch = 19, cex = 0.01)
lm1 = lm(abs(log2.difference)~log2FoldChange, data = oot)
abline(lm1, col = 'red')
plot(absDiff~log2FoldChange, data = oot, xlim = rev(c(-2,2)), ylim = c(.2,.37), col = 'grey')
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = T, scol = 'black', lwd = 1, pch = 19, cex = 0.01)


#plot counts as barplot
counts = plot.counts(ott, 'log2FoldChange', 'adj.p', 0.01, 1/8)
head(counts)
...
quartz()
plot(Fop~log2FoldChange, data = odat3)
lm1 = lm(Fop~log2FoldChange, data = odat3)
summary(lm1)
abline(lm1, col = 'red')

par(mfrow = c(1,1))
window_data = window_plot(1/20, 'log2FoldChange', 'Fop', odat3, 10)
...
head(odat3)
met = odat3[odat3$log2FoldChange > 0,]
ub = odat3[odat3$log2FoldChange < 0,]
xs = c(median(met$log2FoldChange), median(ub$log2FoldChange))
...
quartz()
par(mfrow = c(1,1))
window_data = window_plot(1/8, 'log2FoldChange', 'Fop', no.rib, 10)
...
met = no.rib[no.rib$log2FoldChange > 0,]
ub = no.rib[no.rib$log2FoldChange < 0,]
xs = c(median(met$log2FoldChange), median(ub$log2FoldChange))
mns = c(mean(met$Fop), mean(ub$Fop))
ses = c(std.error(met$Fop), std.error(ub$Fop))
...
dnds = read.table('/Users/grovesdixon/Documents/lab_files/projects/metaTranscriptomes/data_files/old_data_files/pair-wise_dNdS_Adigitifer.txt', header = T)#for digitifera
#UPLOAD THE ORTHOLOG DATA
orthos = read.table('/Users/grovesdixon/Documents/lab_files/projects/metaTranscriptomes/data_files/old_data_files/Adigitifera_CDS_Ortholog_Table_e5_hp50_c2.txt', header = T)##best
...
plot_list_of_species = function(speciesList){
  for (species in speciesList){
    print(species)
    dnds.dat = merge.dats(mdat, read.table(paste("/Users/grovesdixon/Documents/lab_files/projects/metaTranscriptomes/data_files/old_data_files/pair-wise_dNdS_", species, '.txt', sep = ""), header = T), orthos, iso2seq, MUT.TYPE, FILT)
    print(head(dnds.dat))
    N = nrow(dnds.dat)
...
for (DUDE in speciesList){
  ...
  window.dat = plot.mut(dnds.dat, MUT.TYPE, 'cpgOE', MUT.TYPE, 'log2FoldChange', species, FALSE, 0.65)
}
