...
plot(log2FoldChange~cpgOE, data = cpg.dat, pch = 1, cex = 0.25, ylab = expression(paste("log"["2"]," Fold Change")), xlab = expression("CpG"["O/E"]), xlim = c(0,1.5), ylim = c(-4, 6))
#PLOT CORRELATION WITH CpG COUNTS
plot(log2FoldChange~CpG, data = cpg.dat, pch = 1, cex = .2, ylab = expression(paste("log"["2"]," Fold Change")), xlab = expression("CpG (counts within contig)"))
...
hist(mdat$log2FoldChange, breaks = 50, prob = T)
...
require(plotrix)
#READ IN THE DATA
x = read.table('process2iso.txt', header = F)
...
par(mar=c(3,14,1,0))
plotCI(y$means, 1:length(y$means), uiw = y$st.errs, err="x",yaxt="n", pch=19, lwd=2, sfrac = .0075, axes = F, ylab = "")
axis(2, at = 1:nrow(y), labels = F, tick = T)
axis(1, labels = T)
y.labels = paste(y$ps," (",y$Ns,")", sep = "")
...
#THIS REQUIRES THAT WE OUTPUT THE DATA IN A PARTICULAR FORMAT. THEN ANALYZE USING GO_MWU.R
out = data.frame(mdat$EST, mdat$log2FoldChange)
colnames(out) = c('gene', 'log2FoldChange')
write.table(out, "/Users/grovesdixon/Documents/lab_files/projects/metaTranscriptomes/go/MBD_foldChangeForGO.txt", sep = ',', quote = F, row.names = F)
...
Cutoff = 10
window_data = window_plot(1/8, 'log2FoldChange', 'grandmeans', tdat, Cutoff) ## data for Carly's RNA seq
...
#PLOT THE POINTS
plot(mn~x,data=window_data, main="", pch = 1, cex = 1, axes = F, cex.lab = 1, ylim = c(4.2, 4.5), xlim = rev(range(window_data$x)))
axis(1)
axis(2)
#PLOT THE ERROR BARS
...
par(mfrow = c(1,1))
window_data = window_plot(1/12, 'log2FoldChange', 'Fop', odat3, 10)
...
plotCI()
loess_fit <- loess(window_data$mn~window_data$x, span = 1, se = T)
lines(window_data$x, predict(loess_fit),col="red",lwd=1)

window_data = window_plot(1/12, 'log2FoldChange', 'tot', odat3, 10) 
plotCI(window_data$x, window_data$mn, uiw = window_data$sterr, liw = window_data$sterr, add = F, scol = 'black', lwd = 1, pch = 19, cex = 0.01)
...
merge.dats = function(met.dat, dnds.dat, ortho.dat, iso2seq, mut.column, remove.outliers){
  ...
  sub3 = data.frame(sub2$EST, sub2$Amillepora, sub2$dN, sub2$dS)
  colnames(sub3) = c("digEST", "EST", "dN", "dS")
  sub4 = merge(iso2seq, sub3, by = 'EST')
  colnames(sub4) = c('isotig', 'EST', 'digEST', 'dN', 'dS')
  head(sub4)
...
plot_list_of_species(SPECIESLIST)
...
quartz()
par(mfrow = c(2,4))
non.acro = c('Pastreoide', 'Pcarnosus', 'Pdamicorni', 'Spistillat')
...
plot.mut.stackF = function(speciesList, mut.ylim, Ycol, Xcol, XLAB, YLAB, MAIN, rev, SEPERATOR, XLIM){
  par(xpd=F)
...
MUT.TYPE = 'dN'
MUT.YLIM = c(0.075, 0.11)
YLAB = "dN"
NON.ACROPORID <- c('Pastreoide', 'Mcavernosa', 'Pcarnosus', 'Pdaedalea', 'Pstrigosa', 'Fscutaria', 'Mauretenra', 'Pdamicorni', 'Spistillat', 'Shystrix')
NON.ACROPORID <- c('Pastreoide', 'Mcavernosa', 'Pcarnosus', 'Pdaedalea', 'Pstrigosa', 'Fscutaria', 'Mauretenra', 'Pdamicorni', 'Spistillata', 'Shystrix')
plot.mut.stackF(NON.ACROPORID, MUT.YLIM, MUT.TYPE, 'log2FoldChange', XLAB, YLAB, species, TRUE, 0, c(2, -2))
